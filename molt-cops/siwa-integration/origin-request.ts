 ...
 *
 * Priority (updated for SIWA integration):
 *   1. WAF headers → human or verified bot? → free
 *   2. SIWA session token → pre-verified identity + trust (LOCAL, ~2ms)
 *   3. SIWA raw auth → verify signature + resolve trust (one-time RPC)
 *   4. X-Agent-Id header → ERC-8004 lookup → trust score → tier (SLOW, ~500ms)
 *   5. Payment-Signature → has wallet but no identity → CAUTION
 *   6. Nothing → unknown agent → WARNING (highest price)
 *
 * The SIWA fast-path (step 2) is the key optimization. Once an agent
 * authenticates via SIWA, it gets a session token containing its trust
 * tier. Subsequent requests use the token — no RPC calls, no chain
 * queries. Identity resolution drops from ~500ms to ~2ms.
 */
...
interface AgentIdentity {
  ...
  authMethod: "siwa_session" | "siwa_direct" | "erc8004_lookup" | "wallet_only" | "none" | "waf";
}

/**
 * Verify a MoltCops session token (issued after SIWA authentication).
 * Pure local HMAC check — no network calls.
 */
function verifySIWASessionToken(
  token: string
): { valid: true; payload: Record<string, any> } | { valid: false } {
  try {
    const [header, payloadStr, sig] = token.split(".");
    if (!header || !payloadStr || !sig) return { valid: false };
...
/**
 * Verify a raw SIWA authentication message + signature.
 * Local cryptographic operation (ethers.verifyMessage) — no RPC.
 * Returns the recovered address and parsed agent ID.
 */
function verifySIWASignature(
  message: string,
  signature: string,
  expectedDomain: string
): { valid: true; address: string; agentId: string; chainId: number } | { valid: false } {
  try {
    // Parse SIWA message fields
    const lines = message.split("\n").map((l: string) => l.trim());
    const domainMatch = lines[0]?.match(
      /^(.+) wants you to sign in with your agent account:$/
    );
    if (!domainMatch || domainMatch[1] !== expectedDomain) return { valid: false };

    const address = lines[1];
    if (!ethers.isAddress(address)) return { valid: false };

    // Extract key-value fields
    const fields: Record<string, string> = {};
    for (const line of lines) {
      const kv = line.match(/^(\w[\w\s]+):\s+(.+)$/);
      if (kv) fields[kv[1].toLowerCase().replace(/\s+/g, "_")] = kv[2];
    }

    const agentId = fields["agent_id"] || "";
    const chainId = parseInt(fields["chain_id"] || "8453");
    const issuedAt = fields["issued_at"];

    // Check freshness (5 minute window)
    if (issuedAt) {
      const age = Date.now() - new Date(issuedAt).getTime();
      if (age > 300000) return { valid: false }; // Expired
    }

    // Recover signer — THIS is the cryptographic proof
    const recovered = ethers.verifyMessage(message, signature);
    if (recovered.toLowerCase() !== address.toLowerCase()) return { valid: false };

    return { valid: true, address, agentId, chainId };
  } catch {
    return { valid: false };
  }
}
...
async function resolveIdentity(
  ...
  // ── Step 2: SIWA Session Token (FAST PATH — ~2ms, no RPC) ──
  // If the agent previously authenticated via SIWA, it has a session
  // token containing its pre-resolved trust tier. Pure local verification.
  const authHeader = headers["authorization"] || "";
  if (authHeader.startsWith("Bearer ")) {
    const token = authHeader.replace("Bearer ", "");
    const result = verifySIWASessionToken(token);
    if (result.valid) {
      const p = result.payload;
      return {
        tier: p.tier as TrustTier || TrustTier.CAUTION,
        agentId: p.sub || null,
        walletAddress: p.addr || null,
        trustScore: p.score || 50,
        isOperative: p.op === true,
        reputationCount: 0, // Not stored in token — query if needed
        authMethod: "siwa_session",
      };
    }
    // Invalid token — fall through, don't reject (might be a different auth scheme)
  }

  // ── Step 3: SIWA Direct Auth (verify signature, then one-time RPC) ──
  // Agent sends the raw SIWA message + signature in headers.
  // We verify the signature locally (fast) then do one RPC call for trust data.
  const siwaMessage = headers["x-siwa-message"] || null;
  const siwaSignature = headers["x-siwa-signature"] || null;
  if (siwaMessage && siwaSignature) {
    const decoded = Buffer.from(siwaMessage, "base64").toString("utf-8");
    const domain = headers["host"] || "api.moltcops.com";
    const siwaResult = verifySIWASignature(decoded, siwaSignature, domain);

    if (siwaResult.valid) {
      // SIWA verified — now do the on-chain trust lookup (once per session)
      try {
        const profile = await lookupAgentOnChain(
          siwaResult.agentId,
          siwaResult.address
        );
        return { ...profile, authMethod: "siwa_direct" };
      } catch {
        // Chain lookup failed but SIWA is valid — give baseline trust
        return {
          ...
          isOperative: false,
          reputationCount: 0,
          authMethod: "siwa_direct",
        };
      }
    }
  }

  // ── Step 4: Legacy ERC-8004 Header Lookup (SLOW — ~500ms RPC) ──
  // For agents that haven't adopted SIWA yet. Custom headers.
  ...
  if (agentId) {
    try {
      ...
      return { ...profile, authMethod: "erc8004_lookup" };
    } catch (err) {
      console.warn(`ERC-8004 lookup failed for agent ${agentId}:`, err);
    }
  }

  // ── Step 5: Has payment signature but no identity ──
