...
**Version:** 2.1
**Status:** Revised — incorporates production review feedback + ENS identity layer
**Change Summary:** 7 architectural corrections + ENS integration via `digitalyuan.eth` and `ecny.eth` (see Section 13: Changelog)
...
class BankAdapter(ABC):
    ...
    async def reverse_fx_leg(
        ...
        """
        v2: Reverse an off-ledger FX leg (e.g., if Leg 2 fails after Leg 1 succeeds).
        Must:
...
class HSBCHKAdapter(BankAdapter):
    ...
    async def reverse_fx_leg(self, leg_confirmation):
        if leg_confirmation.leg_type == LegType.OFF_LEDGER:
            return await self._reverse_traditional_fx(leg_confirmation)
...
class SettlementOrchestrator:
    ...
    async def settle(self, instruction: SettlementInstruction) -> SettlementResult:
        ...
        # ── PHASE 0: Initial Compliance Screen ─────────────────────
        ...
        if rescreen.is_rescreened and not rescreen.result.passed:
            # Sanctions list changed AND counterparty now flagged
    ...
    def get_config(self, timestamp: datetime) -> ToleranceConfig:
        ...
        if window.liquidity == Liquidity.HIGH:
            return ToleranceConfig(
                max_tolerance_bps=15,      # Tight — deep liquidity
            ...
            return ToleranceConfig(
                max_tolerance_bps=35,      # Wider — thinner book
                requote_threshold_bps=15,
                quote_ttl_seconds=20,      # Shorter TTL — rates move faster
                max_requotes=5,
            )
        elif window.liquidity == Liquidity.LOW:
            return ToleranceConfig(
                max_tolerance_bps=60,      # Wide — thin liquidity
            ...
            return ToleranceConfig(
                max_tolerance_bps=0,       # Don't execute
                requote_threshold_bps=0,
                quote_ttl_seconds=0,
                max_requotes=0,
                execution_paused=True,     # Circuit breaker active
...
**v2: State Machine with SETTLED_UNCONFIRMED**
...
LEG1_EXECUTING (AUD → HKD, off-ledger)
  ...
  │               │
  │               └──→ FAIL → LEG1_REVERSING → ROLLED_BACK
  │
...
**Rollback Path (v2: Multi-Leg Reversal):**

```
If Leg 2 fails after Leg 1 succeeded:
  → Reverse Leg 1: bank adapter: reverse_fx_leg(leg1_confirmation)
  → This converts HKD back to AUD (off-ledger reversal)
  → Release original AUD reservation
  → Store state (status: ROLLED_BACK)
  → Emit SettlementRollbackEvent with details of which legs reversed

If Leg 1 fails:
  → Release AUD reservation (no FX to reverse)
  → Store state (status: ROLLED_BACK)
...
class SettlementReceiptAnchor:
    ...
    async def anchor_receipt(
        ...
        subdomain = confirmation.id.hex[:8]  # Short prefix for readability
...
```
ecny.eth
  ├── a1b2c3d4.ecny.eth     → Settlement receipt hash (2026-02-10)
  ├── e5f6g7h8.ecny.eth     → Settlement receipt hash (2026-02-10)
  ├── i9j0k1l2.ecny.eth     → Settlement receipt hash (2026-02-11)
  └── ...
```
...
- [ ] Run chaos test suite against all **6 failure scenarios** (v2)
