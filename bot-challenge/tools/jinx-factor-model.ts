...
async function runFactorModel(): Promise<FactorOutput> {
  ...
  const ledgerPath = path.join(__dirname, "..", "results", "paper-ledger.json");

  let signals: Signal[] = [];
  let regime = "TRENDING_DOWN"; // default fallback

  // Read active signals from paper ledger (match Wren's ACTIVE + sized filter)
  try {
    if (fs.existsSync(ledgerPath)) {
      const ledger = JSON.parse(fs.readFileSync(ledgerPath, "utf-8"));
      const now = Date.now();
      const MAX_AGE = 24 * 60 * 60 * 1000; // 24h staleness

      const raw = ledger.signals || [];
      
      // Get active, non-stale, non-superseded signals
      signals = raw
        .filter((s: any) => {
          const age = now - (s.unixMs || new Date(s.timestamp).getTime() || 0);
          if (age > MAX_AGE) return false;
          if (s.kelly_status === 'STALE' || s.kelly_status === 'SUPERSEDED') return false;
        ...
        .map((s: any) => ({
          ...
          tier: s.tier || (s.confidence === 'high' ? 'HIGH' : s.confidence === 'medium' ? 'MEDIUM' : 'LOW'),
          tool: s.tool,
          fundingAPR: s.annualizedRate ? s.annualizedRate * 100 : undefined,
          priceChange24h: s.priceChange24h ? s.priceChange24h * 100 : undefined,
          allocation: (s.kelly_size || 0) * 100,
          timestamp: s.timestamp,
        }));

      // Read regime from most recent temporal-edge signal
  ...
  console.log(`📊 ${signals.length} active signals loaded | Regime: ${regime}`);
  ...
  const outputPath = path.join(resultsDir, "factor-model-output.json");
  fs.writeFileSync(outputPath, JSON.stringify(output, null, 2));
  
  const date = new Date().toISOString().slice(0, 10);
  const reportPath = path.join(reportsDir, `jinx-factor-${date}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(output, null, 2));
  
  console.log(`Factor model output written to results/ and reports/`);
...
async function main() {
  ...
  console.log("\n" + emailBody);
  console.log(`\n✅ Jinx Factor Model complete — Risk: ${output.overallRisk.score}/100 (${output.overallRisk.grade})`);
}
